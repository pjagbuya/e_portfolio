import Image from 'next/image'

export default function Acad_Works() {
    return (
      <main className="py-6 backdrop-contrast-75" style={{backgroundColor: 'rgba(213,81,162,0.8)' }}>
        <h1 className="flex justify-center font-extrabold text-6xl white-text-shadow">
          Academic Works
        </h1>

        <br/>
        <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 mb-5">
          1.) C Program CLI Checker Project: <a href="https://github.com/pjagbuya/Checkers-DLSU" className="hover:text-white transition-colors"><i>Go To Repo</i></a>
        </h2>
        <div className="flex flex-row gap-5 justify-center mb-5">
            <Image src={"/checkers_1.png"} alt="temp" width={400} height={400}/>
            <Image src={"/checkers_2.png"} alt="temp" width={400} height={400}/>

          </div>
          <div className="flex flex-row gap-5 justify-center">
            <Image src={"/checkers_3.png"} alt="temp" width={400} height={400}/>
            <Image src={"/checkers_4.png"} alt="temp" width={400} height={400}/>
          </div>

        <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 my-5">
          2.) Unity Augmented Reality Health Education Learning App (ARHELA)
        </h2>
        
        
        <div className="flex flex-row gap-5 justify-center mb-5">
            <Image src={"/arhela_1.png"} alt="temp" width={300} height={600}/>
            <Image src={"/arhela_2.png"} alt="temp" width={300} height={600}/>
            <Image src={"/arhela_3.png"} alt="temp" width={300} height={600}/>
          </div>
          <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 my-5">
          3.) Java Object Oriented Vending Machine: <a href="https://github.com/pjagbuya/Vending-Machine" className="hover:text-white transition-colors"><i>Go To Repo</i></a>
        </h2>
        
        
          <div className="flex flex-row gap-5 justify-center mb-5">
            <Image src={"/vend_1.png"} alt="temp" width={500} height={500}/>
            <Image src={"/order_history.png"} alt="temp" width={500} height={500}/>
          </div>

          <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 my-5">
          4.) Wine Data Modelling: <a href="https://github.com/pjagbuya/CSMODEL-Wine" className="hover:text-white transition-colors"><i>Go To Repo</i></a>
        </h2>
        
        
          <div className="flex flex-row gap-5 justify-center mb-5">
            <Image src={"/datamod_1.png"} alt="temp" width={300} height={600}/>
            <Image src={"/datamod_2.png"} alt="temp" width={300} height={600}/>
            <Image src={"/datamod_3.png"} alt="temp" width={300} height={600}/>
          </div>

          <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 my-5">
          5.) Figma Application UX Designing
        </h2>
        
        
          <div className="flex flex-row gap-5 justify-center mb-5">
            <Image src={"/ux_design.png"} alt="temp" width={1200} height={800}/>
          </div>
          <div className="justify-center flex">
            <a className="underline text-violet-300 text-center" href="https://www.figma.com/proto/Ksg5gAgQG5u6UArZR4gz2a/STHCIUX-MCO3?page-id=0%3A1&node-id=3-6&node-type=canvas&viewport=1054%2C451%2C0.24&t=uU0ktnC5lytBQZ72-1&scaling=scale-down&content-scaling=fixed&starting-point-node-id=3%3A6">
            CLICK HERE FOR PROTOTYPE
            </a>
          </div>
        <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 my-5">
          6.) Broadcast and Unicast Socket Programming: <a href="https://github.com/pjagbuya/CSNETWK-MCO/tree/final_version" className="hover:text-white transition-colors"><i>Go To Repo</i></a>
        </h2>
        
        
          <div className="flex flex-row gap-5 justify-center mb-5">
            <Image src={"/broad_uni.png"} alt="temp" width={1200} height={800}/>
          </div>

        <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 my-5">
          7.) Pamahiin, RPG Horror Game (Made in Godot Engine): <a href="https://github.com/DaLouuu/GAMEDES-Final-Game-Project" className="hover:text-white transition-colors"><i>Go To Repo</i></a>
        </h2>
        
        
          <div className="flex flex-row gap-5 justify-center mb-5">
            <Image src={"/Game1.png"} alt="temp" width={500} height={500}/>
            <Image src={"/Game2.png"} alt="temp" width={500} height={500}/>
          </div>


        <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 my-5">
          8.) Supervised Machine Learning Models on Family Income and Expenditure Dataset: <a href="https://github.com/pjagbuya/STINTSYS_ML" className="hover:text-white transition-colors"><i>Go To Repo</i></a>
        </h2>
        
        
          <div className="flex flex-row gap-5 justify-center mb-5">
            <Image src={"/Feature_Coefficients.png"} alt="temp" width={500} height={500}/>
            <Image src={"/ML Training Results.png"} alt="temp" width={500} height={500}/>
          </div>
        <h2 className="w-full bg-white bg-opacity-20 border-2 border-white font-extrabold text-2xl white-text-shadow py-3 px-5 my-5">
          More Website Works:
        </h2>
        
        <div className="flex gap-5 justify-center mb-5">

          <Image src={"/pers_2.png"} alt="temp" width={800} height={600}/>
          
          
        </div>
        <div className="justify-center flex">
          <a href="https://pjagbuya.github.io/PersonalSite/" className="underline text-violet-300 text-center" target="_blank">https://pjagbuya.github.io/PersonalSite/</a>

        </div>
        <div className="mt-5 flex gap-5 justify-center mb-5">

        <Image src={"/pers_1.png"} alt="temp" width={800} height={600}/>
        
      </div>

      <div className="justify-center flex">
      <a href="https://pjagbuya.github.io/cv/" className="underline text-violet-300 text-center" target="_blank">https://pjagbuya.github.io/cv/</a>

      </div>

       <div className="mt-5 flex gap-5 justify-center mb-5">

        <Image src={"/Anki-Transcriptor.png"} alt="temp" width={800} height={600}/>
        
      </div>
<div class="justify-center flex text-slate-900 font-medium">
  Anki-like website based on transcribing your voice, integrated with Postgres Supabase, Google Cloud, and Vercel:
</div>
      <div className="justify-center flex">
      <a href="https://re-kite.vercel.app/login" className="underline text-violet-300 text-center" target="_blank">https://re-kite.vercel.app/login</a>

      </div>
<div class="justify-center flex text-slate-900 font-medium">
  Simple project demo of a booking reservation website
</div>
      <div className="justify-center flex">
      <a href="https://drive.google.com/file/d/1q1spoS7iBC9NxDZy0krJH7TdlnVAkjUJ/view?usp=sharing" className="underline text-violet-300 text-center" target="_blank">https://drive.google.com/file/d/1q1spoS7iBC9NxDZy0krJH7TdlnVAkjUJ/view?usp=sharing</a>

      </div>
      </main>
    )
  }